/*! @file SE3Controller.h
 *
 *  @brief
 *  See SE3Controller class.
 *
 *  @copyright 2016 DJI Research LLC. All rights reserved.
 *  Unauthorized copying of this file via any medium is strictly prohibited.
 *  Proprietary and confidential.
 */

#ifndef DJI_SE3CONTROLLER_H
#define DJI_SE3CONTROLLER_H

#include <Eigen/Dense>
#include <Eigen/Geometry>

#ifdef DEBUG_CONTROLLER
#include <iostream>
#endif

//! @brief SE(3) Position Controller.
//! @details Outer loop of SE(3) Controller as defined in T. Lee, M. Leok, N. McClamroch [CDC2010]. Integral term added per F. Goodarzi, D. Lee, T. Lee [ECC13]
//! Inner Loop is run using DJI Onboard SDK Movement Control
//! As a user, you do not have to interface with these functions.
class SE3Controller {
 public:
  Eigen::Vector4d trackPosition(Eigen::Vector3d xDesired,
                                Eigen::Vector3d xCurrent,
                                Eigen::Vector3d x_dotDesired,
                                Eigen::Vector3d x_dotCurrent,
                                Eigen::Vector3d x_dot_dotDesired,
                                Eigen::Quaterniond qCur,
                                float psiDes);
  SE3Controller();
  void setKp(Eigen::Vector3d userKp) { Kp = userKp; }
  void setKd(Eigen::Vector3d userKd) { Kd = userKd; }
  void setKi(Eigen::Vector3d userKi) { Ki = userKi; }
  void setMass(float userMass) { m = userMass; }
  void setThustScale(float thrustScale) { thr_scale = thrustScale; }

 private:
  //! System parameters
  float thr_scale;
  float m;
  float g;

  //! Gains
  Eigen::Vector3d Kp;
  Eigen::Vector3d Kd;
  Eigen::Vector3d Ki;

  //! Integral control addons
  Eigen::Vector3d c1; //! Proportion of position error contributing to the integral error, see ECC13
  double errorSatLimit;
  Eigen::Vector3d errorBuildup;

  Eigen::Vector3d errorSaturation(Eigen::Vector3d rawError);
};

#endif //DJI_SE3CONTROLLER_H